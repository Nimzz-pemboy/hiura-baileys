...
export const makeMessagesRecvSocket = (config) => {
    ...
    const { ev, authState, ws, messageMutex, notificationMutex, receiptMutex, signalRepository, query, upsertMessage, resyncAppState, onUnexpectedError, assertSessions, sendNode, relayMessage, sendReceipt, uploadPreKeys, sendPeerDataOperationMessage, messageRetryManager, userDevicesCache, devicesMutex } = sock;
    ...
    // Ported from baileys 7.0.0-rc14 (previously a stub in hiura-baileys that only
    // logged when the notification was about our own devices and never touched
    // userDevicesCache). Without this, a member's device cache only refreshes when
    // its 5-minute TTL expires — in a large/busy group that means messages can miss
    // a device someone just linked, or keep getting encrypted for one they removed,
    // both of which show up as extra failed sends + retries as membership grows.
    ...
    const handlePrivacyTokenNotification = async (node) => {
        ...
        for (const tokenNode of tokenNodes) {
            ...
            if (type === 'trusted_contact' && content instanceof Buffer) {
                // Freshness guard (ported from baileys 7.0.0-rc14 storeTcTokensFromIqResult):
                // skip writes for out-of-order/duplicate notifications so a stale token
                // can't clobber a fresher one we already stored for this jid.
                const existing = (await authState.keys.get('tctoken', [from]))?.[from];
                ...
                if (!incomingTs || (existingTs > 0 && existingTs > incomingTs)) {
                    logger.debug({ from, timestamp, existingTs }, 'ignoring stale trusted contact token');
                    continue;
                }
                logger.debug({
                    from,
    ...
    const sendMessagesAgain = async (key, ids, retryNode, receiptNode) => {
        ...
        // --- Ported from baileys 7.0.0-rc14: fast-path session recovery ---
        // A <receipt type="retry"> can carry a fresh key bundle for the requesting
        // device. When it does, inject it directly instead of tearing the session
        // down and waiting for a full renegotiation round trip (fewer retries,
        // faster recovery — this matters most in busy groups where a stuck
        // session otherwise keeps re-triggering retries from the same device).
        ...
        if (receiptNode) {
            ...
            if (!injectedFromBundle) {
                ...
                if (typeof receivedRegId === 'number' && Number.isInteger(receivedRegId) && signalRepository.getSessionInfo) {
            ...
            // Base-key collision check: if the session's base key hasn't changed
            // across repeated retries of the same message, the session is stuck
            // rather than genuinely renegotiating — force a fresh one.
        ...
        // Check if we should recreate session for this retry (hiura's existing
        // count/timeout heuristic — skipped when a bundle already fixed the
        // session above, no need to also tear it down again).
        let shouldRecreateSession = false;
        let recreateReason = '';
        if (enableAutoSessionRecreation && messageRetryManager && !injectedFromBundle) {
            // Read the explicit Signal error code WA sends on the <retry> node (e.g. Bad MAC).
            // A MAC error means the session is definitely out of sync, so it forces immediate
            // recreation below even on the very first retry — don't wait for retryCount > 1.
            const errorCode = messageRetryManager.parseRetryErrorCode(retryNode?.attrs?.error);
            const isMacError = messageRetryManager.isMacError(errorCode);
            if (retryCount > 1 || isMacError) {
                try {
                    ...
                    recreateReason = result.reason;
                    if (shouldRecreateSession) {
                        logger.debug({ participant, retryCount, errorCode, reason: recreateReason }, 'recreating session for outgoing retry');
                        await authState.keys.set({ session: { [sessionId]: null } });
                    }
                }
                catch (error) {
    ...
    // Messages, receipts and notifications each have their own mutex (messageMutex /
    // receiptMutex / notificationMutex) and so can genuinely run concurrently with one
    // another. ev.buffer()/ev.flush() used directly (as opposed to ev.createBufferedFunction())
    // are a plain on/off flag, not reference-counted — so with two of these in flight at
    // once, whichever one finishes first calls ev.flush() and turns buffering off under the
    // other one's feet: the still-running call's own ev.flush() then becomes a no-op, its
    // events stop being batched with the rest of that node's updates, and consolidated
    // events (e.g. messages.upsert) can be split apart or emitted out of order — which shows
    // up as delayed/inconsistent event delivery under load (busy groups, catching up after a
    // reconnect). ev.createBufferedFunction() wraps the same buffer/flush pair but ref-counts
    // concurrent callers and only flushes once every one of them has finished, so it's safe
    // to use here.
    const processNodeWithBuffer = ev.createBufferedFunction
        ? (node, identifier, exec) => ev.createBufferedFunction(() => exec(node, false).catch(err => onUnexpectedError(err, identifier)))()
        : async (node, identifier, exec) => {
            ev.buffer();
            await execTask();
            ev.flush();
            function execTask() {
