...
function normalizeButtons(buttons) {
  ...
  return buttons.map(btn => {
    ...
    // location_button shorthand, same as Socket/hiura.js's normalizeButtons.
    if (btn.type === 'location' || btn.type === 'send_location') return { name: 'send_location', buttonParamsJson: JSON.stringify({}) };
    return btn;
  });
}
