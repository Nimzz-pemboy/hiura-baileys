...
export class Hiura {
  ...
  async handleLocationButtons(content, _jid, _quoted) {
    ...
    const {
      ...
      buttons = [], jpegThumbnail
    } = data;
    let thumb;
    if (Buffer.isBuffer(jpegThumbnail)) {
      thumb = jpegThumbnail;
    } else if (jpegThumbnail && typeof jpegThumbnail === "object" && jpegThumbnail.url) {
      const { imageMessage } = await generateWAMessageContent({ image: { url: jpegThumbnail.url } }, { upload: this.waUploadToServer });
      thumb = imageMessage?.jpegThumbnail;
    }
    const rawButtons = (buttons || []).map((btn, i) => ({
      buttonId: btn.id || btn.buttonId || `btn_${i}`,
