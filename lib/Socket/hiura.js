...
export class Hiura {
  ...
  detectType(content) {
    ...
    if (content.type === 'location_buttons' || content.locationButtons) return "LOCATION_BUTTONS";
  ...
  normalizeButtons(buttons) {
    ...
    return buttons.map(btn => {
      ...
      // location_button shorthand — { type: 'location' } or { type: 'send_location' }.
      // Without this case it fell through to `return btn` with no `name` at all,
      // which getButtonType()/getButtonArgs() then can't recognize as anything.
      if (btn.type === 'location' || btn.type === 'send_location') {
  ...
  // A "location_buttons" message: a location shown as the message header,
  // with a row of tappable buttons underneath it — different from a
  // send_location *request* button (see normalizeButtons' 'location'
  // shorthand for that). Accepts either fields directly on `content`
  // ({ type: 'location_buttons', lat, lng, name, address, buttons, ... })
  // or nested under `content.locationButtons`.
  //
  // Note on button shape: this used to build a legacy buttonsMessage
  // ({ buttonId, buttonText, type: 1 }, headerType: 6) — that's the OLD
  // WhatsApp protocol, and it's what levvleys v1's buttonLocation still
  // does too. WhatsApp renders that as stacked quick-reply-style buttons,
  // not the floating pill buttons. The floating look comes from
  // InteractiveMessage: header.locationMessage (a real oneof on the
  // Header proto, alongside imageMessage/videoMessage/productMessage) +
  // nativeFlowMessage.buttons — the same interactiveMessage shape
  // handleInteractiveButtons' `location` branch already builds. This now
  // mirrors that path instead, so location_buttons renders the same
  // floating native-flow UI as every other interactive/button message.
  async fetchLocationThumbnail(input) {
    ...
    if (typeof input === "string") {
      try {
        ...
        this.logger?.warn?.({ err: e, input }, "location_buttons: failed to fetch thumbnail");
        return undefined;
      }
    }
    if (!buf) return undefined;
    // Best-effort resize — jpegThumbnail doesn't strictly need to be exactly
    // 300x300, so a resize failure (e.g. Jimp not installed, since it's an
    // optional peer dependency) falls back to the raw fetched bytes instead
    // of dropping the thumbnail entirely.
    try {
  ...
  async handleLocationButtons(content, _jid, _quoted) {
    const data = { ...content, ...(content.locationButtons || {}) };
    const {
      lat = 0, latitude = lat, lng = 0, longitude = lng,
      name = "", address = "", text = "", footer = "",
      title, subtitle,
      buttons = [], jpegThumbnail, thumbnail
    } = data;
    const thumb = await this.fetchLocationThumbnail(jpegThumbnail ?? thumbnail);
    return {
      viewOnceMessage: {
        message: {
