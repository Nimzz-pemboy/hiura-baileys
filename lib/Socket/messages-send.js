...
export const makeMessagesSocket = (config) => {
    ...
    // Ported from baileys 7.0.0-rc14. Serializes writes to userDevicesCache.
    // Without this, concurrent USync device-list refreshes (common in large/busy
    // groups where many participants' messages trigger overlapping lookups) can
    // race and interleave, leading to redundant network round trips instead of
    // reusing what a parallel refresh already fetched.
    ...
    const getButtonType = (message) => {
        // handleProduct/handleLocationButtons/handleCarousel/handleInteractive/
        // handleInteractiveButtons all wrap their content in viewOnceMessage
        // (matching the wire format WhatsApp itself uses for these types), so
        // message.interactiveMessage lives one level deeper than the raw
        // top-level object passed in here. Without unwrapping first, every one
        // of those checks below silently missed and this returned null —
        // meaning getButtonArgs() never ran and the biz/native_flow node
        // (with the actual_actors/host_storage/privacy_mode_ts attrs +
        // quality_control node WhatsApp expects) never got attached at all.
        ...
        if (m.interactiveMessage?.nativeFlowMessage) return 'native_flow';
        if (m.interactiveMessage?.shopStorefrontMessage) return 'shop';
        if (m.interactiveMessage?.collectionMessage) return 'collection';
        if (m.interactiveMessage?.carouselMessage) return 'carousel';
        if (m.interactiveMessage) return 'interactive';
        return null;
    };
    // Maps a message's interactiveMessage/nativeFlowMessage/buttonsMessage shape
    // to the WA stanza "type" the receipt/ack logic downstream expects.
    // Audited against the levvleys fork and upstream baileys: hiura-baileys
    // already covers everything levvleys does here, plus shop/collection/
    // carousel/template dispatch that levvleys does not have.
    const getButtonArgs = (message) => {
        const m = normalizeMessageContent(message) || message || {};
        const nativeFlow = m.interactiveMessage?.nativeFlowMessage;
        const firstButtonName = nativeFlow?.buttons?.[0]?.name;
        // Native-flow button names that require the special <biz><interactive>
        // <native_flow> stanza wrapper instead of the generic buttons/list path —
        // without this, WhatsApp's servers will not render/deliver these button
        // types correctly. `send_location` (the "share your location" request
        // button, i.e. what's referred to as "location_button") is included and
        ...
        } else if (nativeFlow || m.buttonsMessage || m.templateMessage || m.interactiveMessage) {
      ...
      if (
        ...
        switch (messageType) {
          ...
          case "PRODUCT": {
            ...
            const productButtonType = getButtonType(productMsg.message);
            return await relayMessage(jid, productMsg.message, {
              messageId: productMsg.key.id,
              additionalNodes: productButtonType ? [getButtonArgs(productMsg.message)] : options.additionalNodes,
            });
          }
          case "LOCATION_BUTTONS": {
            const locationButtonsContent = await _hiura.handleLocationButtons(
              content,
              jid,
              quoted,
            );
            const locationButtonsMsg = await generateWAMessageFromContent(
              jid,
              locationButtonsContent,
              { quoted, userJid },
            );
            const locationButtonsType = getButtonType(locationButtonsMsg.message);
            return await relayMessage(jid, locationButtonsMsg.message, {
              messageId: locationButtonsMsg.key.id,
              additionalNodes: locationButtonsType ? [getButtonArgs(locationButtonsMsg.message)] : options.additionalNodes,
            });
          }
          case "CAROUSEL": {
            ...
            const carouselButtonType = getButtonType(carouselMsg.message);
            return await relayMessage(jid, carouselMsg.message, {
              messageId: carouselMsg.key.id,
              additionalNodes: carouselButtonType ? [getButtonArgs(carouselMsg.message)] : options.additionalNodes,
            });
          }
          case "INTERACTIVE": {
            ...
            const interactiveButtonType = getButtonType(interactiveMsg.message);
            return await relayMessage(jid, interactiveMsg.message, {
              messageId: interactiveMsg.key.id,
              additionalNodes: interactiveButtonType ? [getButtonArgs(interactiveMsg.message)] : options.additionalNodes,
            });
          }
          case "INTERACTIVE_BUTTONS": {
            ...
            // This is the send_location / location_button path (and every
            // other native-flow button: quick_reply, cta_url, cta_catalog,
            // mpm, etc). getButtonArgs() picks the specific <biz><native_flow
            // name="..."> WhatsApp requires per button type — without this,
            // the message sends but the button doesn't render/function.
            const ibButtonType = getButtonType(ibMsg.message);
            return await relayMessage(jid, ibMsg.message, {
              messageId: ibMsg.key.id,
              additionalNodes: ibButtonType ? [getButtonArgs(ibMsg.message)] : options.additionalNodes,
            });
          }
          case "ALBUM":
        ...
        const fallbackButtonType = getButtonType(fullMsg.message);
        if (fallbackButtonType) {
          additionalNodes.push(getButtonArgs(fullMsg.message));
        }
        await relayMessage(jid, fullMsg.message, {
          ...
          additionalNodes: (isAiMsg || fallbackButtonType) ? additionalNodes : options.additionalNodes,
        });
