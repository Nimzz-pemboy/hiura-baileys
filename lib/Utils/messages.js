...
import { downloadContentFromMessage, encryptedStream, extractImageThumb, prepareStream, generateThumbnail, getAudioDuration, getAudioWaveform, getStream, toBuffer } from './messages-media.js';
...
/** Resolves a URL, file path, or Buffer into a small JPEG thumbnail for a location button header. */
async function processLocationThumbnail(input, width = 300) {
    let bufferOrFilePath;
    if (typeof input === 'string' && (input.startsWith('http://') || input.startsWith('https://'))) {
        const { stream } = await getStream({ url: input });
        bufferOrFilePath = await toBuffer(stream);
    }
    else if (typeof input === 'string') {
        bufferOrFilePath = await fs.readFile(input);
    }
    else if (Buffer.isBuffer(input)) {
        bufferOrFilePath = input;
    }
    else {
        throw new Boom('buttonLocation thumbnail must be a URL, file path or Buffer', { statusCode: 400 });
    }
    const { buffer } = await extractImageThumb(bufferOrFilePath, width);
    return buffer;
}
/** Normalizes a shorthand button object (or an already native-flow one) into WA's native-flow button shape. */
function normalizeLocationButton(btn) {
    if (btn && typeof btn.name === 'string' && typeof btn.buttonParamsJson !== 'undefined') {
        // already native-flow shaped — pass through as-is
        return btn;
    }
    return {
        name: 'quick_reply',
        buttonParamsJson: JSON.stringify({
            display_text: btn?.displayText || btn?.text || btn?.name || 'Button',
            id: btn?.id || btn?.buttonId || 'btn'
        })
    };
}
...
export const generateWAMessageContent = async (message, options) => {
    ...
    if (hasNonNullishProperty(message, 'text')) {
        ...
        // Location card with buttons, e.g.:
        //   sock.sendMessage(jid, { buttonLocation: {
        //     latitude, longitude, name, address, jpegThumbnail,
        //     text, footer, extraButtons: [{ id, displayText }],
        //     listMenu: [{ id, title, description }], listButtonText, listSectionTitle
        //   } })
        // Built as a modern nativeFlowMessage/interactiveMessage (not the legacy
        // buttonsMessage type:1 shape) so it actually renders in current WhatsApp.
        const bl = message.buttonLocation;
        const thumbInput = bl.jpegThumbnail || bl.thumbnail;
        const thumb = thumbInput ? await processLocationThumbnail(thumbInput) : undefined;
        const nativeButtons = [];
        ...
        for (const btn of (bl.extraButtons || [])) {
            nativeButtons.push(normalizeLocationButton(btn));
        }
        ...
        if (bl.footer) {
            interactiveMessage.footer = { text: bl.footer };
        }
    ...
    // Generic native-flow button entry point. Any WA native flow button type
    // (quick_reply, cta_url, cta_call, cta_copy, cta_catalog, single_select,
    // send_location, etc.) goes through here — e.g. a location-request button
    // ("location_button"):
    //   interactiveButtons: [{ name: 'send_location', buttonParamsJson: '{}' }]
    // The special stanza wrapping this needs on the wire is handled in
    // Socket/messages-send.js's getButtonArgs() (see nativeFlowSpecials there).
