...
export const makeEventBuffer = (logger) => {
    ...
    return {
        ...
        /** Call on socket close/logout to release timers and cached state. */
        destroy() {
...
eventData, logger) {
    switch (event) {
        case 'messaging-history.set':
            ...
            // Ported from baileys 7.0.0-rc14: history sync can deliver the same
            // group's past-participants across multiple chunks. Merge by group and
            // dedupe by (userJid, leaveTs, leaveReason) instead of letting entries
            // pile up — matters for groups with a long membership history.
