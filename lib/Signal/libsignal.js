...
;(function _suppressSessionNoise() {
    const _isNoise = (...args) => {
        if (typeof args[0] === 'string') {
            const s = args[0];
            if (s.startsWith('Closing session') || s.startsWith('Closing open session') ||
    ...
    process.stdout.write = function(chunk, ...rest) {
        if (typeof chunk === 'string' && (
            chunk.includes('Closing session') || chunk.includes('Closing open session') || chunk.includes('SessionEntry') ||
...
export function makeLibSignalRepository(auth, logger, pnToLIDFunc) {
    ...
    const repository = {
        ...
        // Ported from baileys 7.0.0-rc14. Reads the current open session's base key
        // and registration id without mutating anything — used by sendMessagesAgain()
        // to detect base-key collisions (a sign of session desync) and registration-id
        // mismatches on message retries.
...
function signalStorage({ creds, keys }, lidMapping) {
    ...
    return {
        ...
        // Ported from baileys 7.0.0-rc14. Call on socket close/logout to release
        // the in-memory caches this repository holds (avoids leaking them across
        // reconnects when a new repository instance is created each time).
        close() {
